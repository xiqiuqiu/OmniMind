...
<template>
    <div class="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col gap-3">
        <div class="bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl p-2 flex flex-col gap-2">
            <!-- 适配 -->
            <button @click="props.onFit" class="side-btn group text-blue-500 hover:bg-blue-50 border-blue-100">
                <Focus class="w-5 h-5" />
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.fit') }}</span>
            </button>

            <!-- 布局 -->
            <button @click="props.onResetLayout" class="side-btn group text-purple-500 hover:bg-purple-50 border-purple-100">
                <LayoutDashboard class="w-5 h-5" />
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.layout') }}</span>
            </button>

            <!-- 起点 -->
            <button @click="props.onCenterRoot" class="side-btn group text-orange-500 hover:bg-orange-50 border-orange-100">
                <Target class="w-5 h-5" />
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.center') }}</span>
            </button>
            ...
            >
                ...
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.hierarchicalDragging') }}</span>
            </button>
            ...
            >
                <Map class="w-5 h-5" />
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.map') }}</span>
            </button>
            ...
            <div class="relative group p-2 flex flex-col items-center gap-1 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.edge') }}</span>
            </div>

            <!-- 连线类型 -->
            <div data-side-menu="true" class="relative">
                <button @click="toggleEdgeTypeMenu" class="side-btn group" :class="isEdgeTypeMenuOpen ? 'bg-slate-100 text-slate-900' : 'text-slate-400'">
            ...
            <!-- 背景类型 -->
            <div data-side-menu="true" class="relative">
                <button @click="toggleBackgroundMenu" class="side-btn group" :class="isBackgroundMenuOpen ? 'bg-slate-100 text-slate-900' : 'text-slate-400'">
                    <Grid class="w-5 h-5" />
                    <span class="custom-tooltip custom-tooltip-right">{{ props.t('nav.lines') }}</span>
                </button>
                <div
