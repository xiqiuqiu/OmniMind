...
const currentBackgroundLabel = computed(() => {
  const option = backgroundOptions.find(
    (o) => o.value === props.config.backgroundVariant,
  );
  return option
    ? props.t(option.labelKey)
    : String(props.config.backgroundVariant);
});
...
<template>
  ...
  >
    <div class="flex items-center gap-2 md:gap-6 flex-grow mr-2">
      ...
      <!-- 搜索框 -->
      <div
        class="hidden lg:flex items-center relative flex-grow max-w-xs group"
      >
        <Search
        ...
        <input
          ...
          @input="
            (e) =>
              props.onUpdateSearchQuery((e.target as HTMLInputElement).value)
          "
      ...
      <div
        class="h-6 w-[1px] bg-slate-200 mx-1 md:mx-2 flex-shrink-0 hidden lg:block"
      ></div>
      ...
      <div class="hidden md:flex items-center gap-2">
        ...
        <button
          @click="checkAccess() && props.onGenerateSummary()"
          class="toolbar-btn text-orange-600 hover:bg-orange-50 border-orange-100 flex-shrink-0"
        >
        ...
        <button
          @click="props.onStartNewSession"
          class="toolbar-btn text-red-500 hover:bg-red-50 border-red-100 flex-shrink-0"
        >
    ...
    <div class="flex items-center gap-1 md:gap-2 flex-shrink-0">
      <div
        class="hidden md:block h-4 w-[1px] bg-slate-200 mx-1 flex-shrink-0"
      ></div>

      <button
        @click="checkAccess() && props.onToggleChat()"
        class="hidden md:flex p-1.5 md:p-2 hover:bg-orange-50 rounded-md transition-colors text-orange-500 items-center gap-1"
      >
      ...
      >
        <Globe class="w-3.5 h-3.5 md:w-4 h-4" />
        {{ props.locale === "zh" ? "EN" : "ZH" }}
      </button>
      <!-- 项目选择器 -->
      ...
      <button
        ...
        class="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-orange-400 to-rose-400 text-white rounded-lg text-xs font-bold hover:from-orange-500 hover:to-rose-500 transition-all shadow-sm"
      >
      ...
      <div
        v-else
        class="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-2 py-1 shadow-sm"
      >
        <div
          class="w-5 h-5 bg-gradient-to-br from-orange-400 to-rose-400 rounded-full flex items-center justify-center text-white text-[10px] font-bold"
        >
          {{ props.user?.email?.charAt(0)?.toUpperCase() || "U" }}
        </div>
        <span
          class="text-xs text-slate-600 font-medium max-w-[80px] truncate hidden sm:inline"
        >
  ...
  >
    ...
    >
      <button
        @click="callAndClose(props.onFit)"
        class="toolbar-btn text-blue-500 hover:bg-blue-50 border-blue-100"
      >
        <Focus class="w-4 h-4" />
      ...
      <button
        @click="callAndClose(props.onResetLayout)"
        class="toolbar-btn text-purple-500 hover:bg-purple-50 border-purple-100"
      >
      ...
      <button
        @click="callAndClose(props.onCenterRoot)"
        class="toolbar-btn text-orange-500 hover:bg-orange-50 border-orange-100"
      >
      ...
      <button
        @click="callAndClose(props.onStartNewSession)"
        class="toolbar-btn text-red-500 hover:bg-red-50 border-red-100"
      >
      ...
      <div
        class="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-100"
      >
        <Palette class="w-4 h-4 text-slate-400" />
        <input
          type="color"
          v-model="props.config.edgeColor"
          class="w-4 h-4 rounded cursor-pointer bg-transparent border-none"
        />
      ...
      <div data-edge-type-menu="true" class="relative">
        <button
          type="button"
          class="toolbar-select flex items-center gap-2"
      ...
      <button
        @click="callAndClose(props.onGenerateSummary)"
        class="toolbar-btn text-orange-600 hover:bg-orange-50 border-orange-100"
      >
      ...
      <button
        @click="callAndClose(props.onExportMarkdown)"
